...
def map_lang_code(raw_code: str) -> str:
    r = raw_code.strip().lower()
    if r == "kr":
        return "ko"
    if r == "jp":
        return "ja"
...
def main():
    ...
    parser.add_argument(
        "--wav-scp", type=str, required=True, help="wav.scp 파일 경로"
    )
    parser.add_argument(
        "--rttm-file", type=str, required=True, help="RTTM 파일 경로"
    )
    parser.add_argument(
        "--model-name", type=str, default="large-v3", help="Whisper 모델 이름"
    )
    parser.add_argument(
        "--language", type=str, default="ko",
        help="언어 코드 입력 (예: KR, JP, CN, ko, ja, zh 등)"
    )
    parser.add_argument(
        "--output-dir", type=str, required=True, help="STT 결과 저장 디렉토리"
    )
    ...
    mapped_lang = map_lang_code(args.language)

    all_transcriptions = transcribe_audio_segments(
        wav_mapping,
        segments_by_utt,
