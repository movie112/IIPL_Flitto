...
from .utils.rttm_load import rttm_load
...
class SpeakerDiarization(pl.LightningModule):
    ...
    def test_step(self, batch, batch_index):
        ...
        version = "ver_0"
        save_dir_parnt = f"/nas_homes/byeonggeuk/model/FS-EEND/visualize/{version}"
        ...
        np.save(f"{save_dir_parnt}/labels/label_{rec[0]}.npy", labels_realspk[0].detach().cpu().numpy())
        np.save(f"{save_dir_parnt}/embs/emb_{rec[0]}.npy", embs[0].detach().cpu().numpy())
        np.save(f"{save_dir_parnt}/attractors/attractor_{rec[0]}.npy", attractors[0].detach().cpu().numpy())
        return stats
